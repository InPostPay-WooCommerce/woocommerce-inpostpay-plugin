...
namespace Isolated\Inpost_Pay\ZipStream\ZipStream;

use Isolated\Inpost_Pay\ZipStream\Psr\Http\Message\StreamInterface;
use Isolated\Inpost_Pay\ZipStream\ZipStream\Exception\EncodingException;
use Isolated\Inpost_Pay\ZipStream\ZipStream\Exception\FileNotFoundException;
use Isolated\Inpost_Pay\ZipStream\ZipStream\Exception\FileNotReadableException;
use Isolated\Inpost_Pay\ZipStream\ZipStream\Exception\OverflowException;
use Isolated\Inpost_Pay\ZipStream\ZipStream\Option\File as FileOptions;
use Isolated\Inpost_Pay\ZipStream\ZipStream\Option\Method;
use Isolated\Inpost_Pay\ZipStream\ZipStream\Option\Version;
class File
{
    ...
    public function addFileHeader() : void
    {
        ...
        if (!\mb_check_encoding($name, 'ASCII') || !\mb_check_encoding($comment, 'ASCII')) {
            ...
            if (!\mb_check_encoding($name, 'UTF-8') || !\mb_check_encoding($comment, 'UTF-8')) {
                throw new EncodingException('File name and comment should use UTF-8 ' . 'if one of them does not fit into ASCII range.');
            }
