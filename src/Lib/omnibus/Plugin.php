...
class Plugin extends Inpost_Plugin {
	...
	public function enqueue_dashboard_scripts() {
		wp_enqueue_style(
			$this->get_plugin_prefix() . '_admin_css',
	...
	private function implement_omnibus() {
		...
		$omnibus
			...
			->action(
				function ( Wc_Product_Aware_Interface $product_aware ) use (
					$product_service
				) {
			...
			->action(
				function (
					Wc_Variation_Aware_Interface $product_aware
				) use (
					$product_service
				) {
			...
			->action(
				function ( Wp_Post_Id_Aware_Interface $post_id_aware ) {
	...
	public function get_option_show_on_listing(): bool {
		return get_option(
			self::$option_show_on_listing_key,
			0
		) == 1;
	}
	...
	public function get_option_show_on_none_discount_products(): bool {
		return get_option(
			self::$option_show_on_none_discount_products_key,
			0
		) == 1;
	}
