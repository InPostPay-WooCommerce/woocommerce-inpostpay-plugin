<?php
/**
 * Order GUI config section view.
 *
 * @package InPost_Pay
 */

$show_order           = (string) get_option( 'izi_show_order' );
$selected_order_place = (string) get_option( 'izi_place_order' );
$selected_align       = (string) get_option( 'izi_align_order' );
?>
<h3>
	<?php esc_html_e( 'Order', 'inpost-pay' ); ?>
</h3>
	...
	<tr>
		...
		<td class="input-tooltip d-flex-align-center">
			<select name="izi_place_order">
				...
				<?php
				$order_places = array(
					'woocommerce_before_order_notes'   => __( 'Before order notes', 'inpost-pay' ),
					'woocommerce_after_order_notes'    => __( 'After order notes', 'inpost-pay' ),
					...
					'woocommerce_review_order_before_cart_contents' => __( 'Before cart contents', 'inpost-pay' ),
					'woocommerce_review_order_after_cart_contents' => __( 'After cart contents', 'inpost-pay' ),
					'woocommerce_review_order_before_shipping' => __( 'Before shipping', 'inpost-pay' ),
					'woocommerce_review_order_after_shipping' => __( 'After shipping', 'inpost-pay' ),
					'woocommerce_review_order_before_order_total' => __( 'Before order total', 'inpost-pay' ),
					'woocommerce_review_order_after_order_total' => __( 'After order total', 'inpost-pay' ),
					'woocommerce_review_order_before_payment' => __( 'Before payment', 'inpost-pay' ),
					'woocommerce_review_order_before_submit' => __( 'Before submit', 'inpost-pay' ),
					'woocommerce_review_order_after_submit' => __( 'After submit', 'inpost-pay' ),
					'woocommerce_review_order_after_payment' => __( 'After payment', 'inpost-pay' ),
				...
				foreach ( $order_places as $value => $label ) {
					$selected = ( $selected_order_place === $value ) ? 'selected' : '';
